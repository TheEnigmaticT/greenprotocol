...
export type ScopedToolCall = ChemicalToolCall | EvidenceToolCall | HazardToolCall | ScreeningToolCall

export interface ToolResult {
  operation: 'chem21' | 'pubchem' | 'rdkit' | 'solvent_evidence' | 'solvent_hazard' | 'solvent_screening'
  ...
  return [
    ...
    },
    {
      ...
      function: {
        ...
        description: 'Look up a scoped chemical’s PubChem properties and GHS profile.',
    ...
    },
    {
      ...
      function: {
        ...
        description: 'Calculate RDKit properties for a scoped, resolved chemical.',
        parameters: parameters({ chemical: scopedChemical }, ['chemical']),
      },
    },
    {
    ...
    },
    {
      ...
      function: {
        name: 'lookup_solvent_hazard_profile',
        description: 'Read the local GHS hazard profile for a locally indexed solvent.',
    ...
    },
    {
      ...
      function: {
        name: 'screen_solvent_candidates',
        description: 'Screen local solvent candidates against a scoped solute and current solvent at an exact temperature.',
        parameters: parameters({
          solute: scopedChemical,
          currentSolvent: scopedChemical,
          temperatureK: { type: 'number' },
        }, ['solute', 'currentSolvent', 'temperatureK']),
      },
    },
  ]
}
...
function requestFor(call: ScopedToolCall): Record<string, unknown> {
  switch (call.name) {
    case 'lookup_chem21_solvent':
      return { operation: 'chem21', chemical_name: call.chemical }
    case 'lookup_pubchem_profile':
      return { operation: 'pubchem', chemical_name: call.chemical }
    case 'calculate_rdkit_properties':
      return { operation: 'rdkit', chemical_name: call.chemical }
    case 'lookup_solvent_hazard_profile':
      return { operation: 'solvent_hazard', solvent: call.solvent }
    case 'screen_solvent_candidates':
      if (!call.canonicalSoluteSmiles) {
        throw new Error('Resolve the scoped solute with PubChem before screening')
      }
      return {
        operation: 'solvent_screening',
        solute_smiles: call.canonicalSoluteSmiles,
        current_solvent: call.currentSolvent,
        temperature_k: call.temperatureK,
      }
    case 'lookup_experimental_solvent_evidence': {
      const request: Record<string, unknown> = {
        operation: 'solvent_evidence',
        mode: call.mode,
        solvent: call.solvent,
        temperature_k: call.temperatureK,
      }
...
export async function executeScopedTool(
  ...
  switch (call.name) {
    case 'lookup_chem21_solvent':
      if (!matches([...scoped, ...LOCAL_INDEXED_SOLVENTS], call.chemical)) {
    ...
    case 'lookup_solvent_hazard_profile':
      ...
      break
    case 'screen_solvent_candidates':
      if (!matches(scoped, call.solute) || !matches(scoped, call.currentSolvent)) {
        throw new Error('Requested chemical is outside this scoped discussion')
      }
    ...
    case 'lookup_experimental_solvent_evidence':
      assertFiniteTemperature(call.temperatureK)
      if (call.mode === 'density') {
        if (!matches(LOCAL_INDEXED_SOLVENTS, call.solvent)) {
          throw new Error('Requested solvent is outside this scoped discussion')
        }
      } else if (!call.solute || !matches(scoped, call.solute) || !matches(scoped, call.solvent)
        || (call.mode === 'mixture_solubility' && (!call.coSolvent || !matches(scoped, call.coSolvent)))) {
        throw new Error('Requested chemical is outside this scoped discussion')
      }
      break
  }
  ...
  if (process.env.CHEMISTRY_SERVICE_TOKEN) {
    headers.set('X-Chemistry-Service-Token', process.env.CHEMISTRY_SERVICE_TOKEN)
  }
  const response = await fetch(`${process.env.CHEMISTRY_SERVICE_URL || 'http://localhost:8000'}/assistant-tools`, {
    method: 'POST',
    headers,
    signal,
    body: JSON.stringify(requestFor(call)),
  })
  if (!response.ok) {
    throw new Error(`Chemistry tool request failed (${response.status})`)
  }
