/**
 * Fail-closed local dense literature retrieval (Ollama nomic-embed-text).
 * Used when GCAI_LOCAL_EVIDENCE_INDEX points at a nomic index so candidate
 * probes do not require a public evidence RPC.
 */
...
export function localEvidenceIndexPath(env: NodeJS.ProcessEnv = process.env): string | null {
...
export async function searchLocalLiteratureEvidence(options: {
  ...
  const dir = options.indexDir ?? localEvidenceIndexPath()
