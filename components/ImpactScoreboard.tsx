  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-sm">
        <span style={{ color: '#F5F5F4' }}>{label}</span>
        <span className="font-[family-name:var(--font-mono)]" style={{ color: '#a3a3a3' }}>
          {originalValue.toFixed(2)} → {greenValue.toFixed(2)} {unit}
        </span>
      </div>
      <div className="space-y-1">
        <div className="h-6 rounded overflow-hidden" style={{ background: '#1B433240' }}>
          <div
            className="h-full rounded transition-all duration-1000 ease-out flex items-center px-2"
            style={{ width: `${originalPct}%`, background: '#EF444480', minWidth: originalPct > 0 ? '2rem' : '0' }}
          >
            <span className="text-xs whitespace-nowrap" style={{ color: '#F5F5F4' }}>Original</span>
          </div>
        </div>
        <div className="h-6 rounded overflow-hidden" style={{ background: '#1B433240' }}>
          <div
            className="h-full rounded transition-all duration-1000 ease-out flex items-center px-2"
            style={{ width: `${greenPct}%`, background: `${color}80`, minWidth: greenPct > 0 ? '2rem' : '0' }}
          >
            <span className="text-xs whitespace-nowrap" style={{ color: '#F5F5F4' }}>Green</span>
          </div>
        </div>
      </div>
    </div>
